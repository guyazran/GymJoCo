...
def episode_from_cfg(cfg: Config) -> EpisodeSpec:
    ...
    strings and values match those of the specification dataclasses from the `gymjoco.episode.specs` sub-package. A
    nested specification may be defined using a configuration dictionary with the keys corresponding to that
    specification's dataclass. All such keys are provided as constants in `gymjoco.defs.cfg_keys`. Specifications that
    can be initialized wiith a single parameter may be defined by that parameter in the configuration.

    >>> from gymjoco.episode import RobotSpec
    >>> episode_from_cfg({
    ...     'scene': 'floatworld',
    ...     'robot': RobotSpec('ur5e'),
    ...     'task': {
    ...         'cls': 'gymjoco.tasks:NullTask'
    ...         # missing 'params' is set as default `{}` as in `gymjoco.episode.specs.task_spec.TaskSpec`
    ...     }
    ... })  # doctest:+ELLIPSIS
...
def scene_spec_from_name_or_cfg(asset_or_cfg: AssetOrConfig) -> SceneSpec:
    ...
    See `gymjoco.episode.cfg_parsing.episode_from_cfg` for more details.
